...
## Next.js App Router: Server Locale Not Initialized

### The Problem

In App Router / RSC projects, initializing locale only in a root layout can still fail for server pages or metadata generation:

```tsx
// ❌ FRAGILE - layout-only init, metadata/page may execute in a different server path
...
1. Server rendering paths are concurrent in App Router.
2. Relying on one component to initialize locale for all other server boundaries is unsafe.
3. Global translation calls can race if locale activation is not explicit per boundary.

### The Solution

1. Initialize locale in both server `layout.tsx` and server `page.tsx` before any translation call.
2. In `initLingui(locale)`, activate locale before binding context.
3. Prefer `useLingui`/`Trans` in shared server components over global `t` calls.
