import { Role, OrderStatus, TransactionType, ProductStatus, UserStatus, BlogStatus, InventoryMovementType, VehicleStatus, NotificationType } from '../enums'
import type { Profile, Category, Product, ProductReview, CustomPrice, Address, Order, OrderItem, Transaction, InventoryItem, InventoryMovement, BlogCategory, BlogPost, SystemConfig, DashboardKPI, RevenuePoint, TopBuyer, Vehicle, Notification, Message, DeliveryRoute } from '../types'
...
// ─── Helpers ──────────────────────────────────────────────
const avatarFor = (name: string) => `https://api.dicebear.com/7.x/initials/svg?seed=${encodeURIComponent(name)}&backgroundColor=ed7628,f59e0b,10b981,3b82f6,8b5cf6,ec4899&radius=50`
const imgFor = (id: number) => `https://picsum.photos/seed/buntech${id}/600/600`
...
const surnames = ['Nguyễn', 'Trần', 'Lê', 'Phạm', 'Hoàng', 'Vũ', 'Phan', 'Bùi', 'Đặng', 'Đỗ', 'Ngô', 'Dương', 'Lý', 'Võ', 'Trương']
const maleMiddle = ['Văn', 'Hữu', 'Đức', 'Minh', 'Quang', 'Thanh', 'Hoàng', 'Bảo', 'Anh', 'Quốc', 'Nhất', 'Phú', 'Tấn', 'Công']
const femaleMiddle = ['Thị', 'Ngọc', 'Thanh', 'Mỹ', 'Hoài', 'Bảo', 'Hồng', 'Kim', 'Thu', 'Quỳnh', 'Phương', 'Mai', 'Hà', 'Diệu']
const maleNames = ['Hùng', 'Nam', 'Dũng', 'Tuấn', 'Khoa', 'Phong', 'Lộc', 'Thắng', 'Cường', 'Hải', 'Bảo', 'Đạt', 'Quân', 'Long', 'Phát', 'Sơn', 'Trí', 'Tài', 'Hiếu', 'Khôi']
const femaleNames = ['Mai', 'Hoa', 'Lan', 'Hà', 'Mỹ', 'Nga', 'Trang', 'Xuân', 'Hồng', 'Vân', 'Kiều', 'Diệu', 'Phương', 'Thảo', 'Linh', 'Thủy', 'Hằng', 'Nhi', 'Tâm', 'Quỳnh']
const driverNames = ['Tài', 'Lái', 'Hùng', 'Nam', 'Dũng', 'Phong', 'Lộc', 'Thắng', 'Cường', 'Hải', 'Đạt', 'Quân', 'Long', 'Phát', 'Sơn', 'Trí', 'Hiếu', 'Khôi', 'Minh', 'Bảo', 'Tân', 'Phú', 'Tấn', 'Công', 'Hòa', 'Thiện', 'Vinh', 'Phước', 'Lộc', 'Khang']
...
const adminProfiles: Profile[] = [
  { id: uid('usr', 1), role: Role.ADMIN, phone: '0901234567', full_name: 'Nguyễn Quang Admin', status: UserStatus.ACTIVE, debt_limit: 0, avatar_url: avatarFor('Nguyễn Quang Admin'), created_at: iso(120), updated_at: iso(1) },
  { id: uid('usr', 2), role: Role.ADMIN, phone: '0901234568', full_name: 'Trần Thị Quản Lý', status: UserStatus.ACTIVE, debt_limit: 0, avatar_url: avatarFor('Trần Thị Quản Lý'), created_at: iso(100), updated_at: iso(2) },
]
...
const productBases: Array<{ name: string; basePrice: number; category: string; unit: string; desc: string }> = [
  { name: 'Bún tươi', basePrice: 25000, category: 'Bún tươi', unit: 'kg', desc: 'Bún tươi làm thủ công mỗi ngày từ gạo tẻ nguyên chất.' },
  { name: 'Bún khô', basePrice: 35000, category: 'Bún khô', unit: 'kg', desc: 'Bún khô phơi nắng tự nhiên, bảo quản được lâu.' },
  { name: 'Phở tươi', basePrice: 40000, category: 'Phở tươi', unit: 'kg', desc: 'Bánh phở tươi mềm dẻo, làm từ gạo tẻ thơm.' },
  { name: 'Miến', basePrice: 45000, category: 'Miến', unit: 'kg', desc: 'Miến dong nguyên chất từ tinh bột khoai lang.' },
  { name: 'Hủ tiếu', basePrice: 38000, category: 'Hủ tiếu', unit: 'kg', desc: 'Hủ tiếu tươi miền Nam, mềm, dẻo.' },
  { name: 'Bánh canh', basePrice: 35000, category: 'Bánh canh', unit: 'kg', desc: 'Bánh canh tươi sợi to, dai, nấu bò hoặc giò heo.' },
  { name: 'Miến dong', basePrice: 48000, category: 'Miến dong', unit: 'kg', desc: 'Miến dong 100% tinh bột khoai lang tự nhiên.' },
  { name: 'Đồ gia dụng', basePrice: 85000, category: 'Đồ gia dụng', unit: 'cái', desc: 'Dụng cụ làm bún thủ công bằng gỗ tre, bền, đẹp.' },
]

const variants = ['sợi nhỏ', 'sợi lớn', 'sợi vừa', 'đặc biệt', 'cao cấp', 'nứt thủ công', 'gạo lứt', 'nghệ', 'trà xanh', 'đen']
const packaging = [
...
function generateProducts(): Product[] {
  ...
  for (const base of productBases) {
    ...
    products.push({
      ...
      category: cat,
      name: base.name,
      slug: `${base.name.toLowerCase().replace(/[^a-z0-9\u00C0-\u1EF9]+/g, '-').replace(/^-|-$/g, '')}`,
    ...
    for (const variant of variants) {
      for (const pkg of packaging) {
        ...
        const price = Math.round((base.basePrice * pkg.priceMult * (1 + variants.indexOf(variant) * 0.05)) / 1000) * 1000
        ...
        products.push({
          ...
          category: cat,
          name,
          slug: name.toLowerCase().replace(/[^a-z0-9\u00C0-\u1EF9]+/g, '-').replace(/^-|-$/g, ''),
...
// ─── Addresses (60+) ─────────────────────────────────────────────
const hcmStreets = [
  'Nguyễn Văn Linh', 'Lê Lợi', 'Trần Hưng Đạo', 'Điện Biên Phủ', 'Pasteur',
  'Cách Mạng Tháng 8', 'Hai Bà Trưng', 'Lý Thường Kiệt', 'Võ Văn Tần',
  'Nguyễn Trãi', 'Tôn Thất Tùng', 'Phạm Văn Đồng', 'Trần Quang Khải',
  'Nguyễn Thái Học', 'Cống Quỳnh', 'Bùi Thị Xuân', 'Ung Văn Khiêm',
  'Lê Quang Định', 'Phan Xích Long', 'Nguyễn Kiệm', 'Lac Long Quân',
  'Tân Sơn', 'Hồng Bàng', 'An Dương Vương', 'Hùng Vương',
]
const hcmWards = ['Phường 1', 'Phường 3', 'Phường 4', 'Phường 5', 'Phường 6', 'Phường 7', 'Phường Bến Nghé', 'Phường Cầu Ông Lãnh', 'Phường Tân Định', 'Phường 25']
const hcmDistricts = ['Quận 1', 'Quận 3', 'Quận 5', 'Quận 10', 'Bình Thạnh', 'Tân Bình', 'Phú Nhuận', 'Gò Vấp']
...
const orderStatusWeights = [
  ...
  OrderStatus.SHIPPING, OrderStatus.SHIPPING, OrderStatus.SHIPPING, OrderStatus.SHIPPING,
  OrderStatus.DELIVERED, OrderStatus.DELIVERED, OrderStatus.DELIVERED, OrderStatus.DELIVERED, OrderStatus.DELIVERED, OrderStatus.DELIVERED, OrderStatus.DELIVERED, OrderStatus.DELIVERED, OrderStatus.DELIVERED,
  OrderStatus.CANCELLED,
]
const orderNotes = ['Giao buổi sáng', 'Giao buổi chiều', 'Giao gấp', '', '', 'Đơn sỉ lớn', 'Khách quen', '', 'Giao trước 9h', '', 'Đóng gói cẩn thận', 'Giao tận nhà']
...
function generateOrders(): Order[] {
  ...
  for (let i = 0; i < 500; i++) {
    ...
    const hasDriver = status !== OrderStatus.PENDING && status !== OrderStatus.CANCELLED && i % 5 !== 0
    ...
    for (let j = 0; j < itemCount; j++) {
      ...
      const qty = ((i + j) % 18) + 2
      const customPrice = mockCustomPrices.find(cp => cp.user_id === cust && cp.product_id === prod.id)
    ...
    orders.push({
      ...
      status,
      total,
      amount_collected: status === OrderStatus.DELIVERED ? total : (status === OrderStatus.SHIPPING ? Math.floor(total * 0.5) : 0),
...
function generateTransactions(): Transaction[] {
  ...
  const types = [TransactionType.DEBT_INCREASE, TransactionType.DEBT_INCREASE, TransactionType.DEBT_PAYMENT, TransactionType.PAYMENT, TransactionType.REFUND]
  for (let i = 0; i < 120; i++) {
    const cust = allCustomerIds[i % allCustomerIds.length]
    const custProfile = mockProfiles.find(p => p.id === cust) || null
    const type = types[i % types.length]
    const order = mockOrders.find(o => o.user_id === cust)
    const amount = type === TransactionType.DEBT_INCREASE ? (order?.total || randInt(200000, 2000000))
      : type === TransactionType.DEBT_PAYMENT ? randInt(100000, 1000000)
      : type === TransactionType.PAYMENT ? randInt(50000, 500000)
    ...
    txns.push({
      ...
      type,
      amount,
      note: type === TransactionType.DEBT_INCREASE ? `Đơn hàng ${order?.id?.slice(0, 8) || ''}`
        : type === TransactionType.DEBT_PAYMENT ? 'Thanh toán tiền nợ'
        : type === TransactionType.PAYMENT ? 'Thu tiền giao hàng'
        : 'Hoàn tiền đơn hủy',
      created_at: iso(i % 90),
    })
  }
...
export const mockReviews: ProductReview[] = Array.from({ length: 60 }, (_, i) => {
  const prod = mockProducts[i % mockProducts.length]
  const cust = customerProfiles[i % customerProfiles.length]
  return {
    id: uid('rev', i + 1),
...
const movementTypes = [InventoryMovementType.IMPORT, InventoryMovementType.IMPORT, InventoryMovementType.EXPORT, InventoryMovementType.LOSS]
const movementNotes = ['Nhập mới từ nhà cung cấp', 'Nhập thêm nguyên liệu', 'Xuất sản xuất', 'Hao hụt tự nhiên', 'Nhập kho', 'Xuất sản xuất theo đơn', 'Kiểm kê điều chỉnh', 'Nhập kho đầu ngày']
...
const blogData: Array<[string, string, string, string, string, number]> = [
  ['BunTech - Hành trình 3 đời làm bún thủ công', 'Câu chuyện gia đình BunTech từ xưởng bún nhỏ đến thương hiệu uy tín.', '<p>Xưởng bún BunTech bắt đầu từ năm 1960 với ông bà nội. Từ những mẻ bún nhỏ làm bằng tay, đến nay chúng tôi đã mở rộng quy mô nhưng vẫn giữ phương pháp thủ công truyền thống.</p><h2>Giá trị cốt lõi</h2><p>Chúng tôi cam kết sử dụng gạo nguyên chất, không chất bảo quản, không chất tẩy trắng.</p>', 'Tin tức', 'https://images.pexels.com/photos/6711698/pexels-photo-6711698.jpeg?auto=compress&cs=tinysrgb&w=800', 20],
  ['Cách phân biệt bún tươi và bún khô', 'Bún tươi và bún khô có gì khác nhau? Cách chọn bún ngon.', '<p>Bún tươi là loại vừa làm xong, sợi mềm, dùng trong ngày. Bún khô là bún đã phơi khô, bảo quản lâu hơn.</p>', 'Kiến thức', 'https://images.pexels.com/photos/6711687/pexels-photo-6711687.jpeg?auto=compress&cs=tinysrgb&w=800', 18],
  ['Công thức bún bò huế chuẩn vị', 'Hướng dẫn nấu bún bò huế thơm ngon với bún tươi BunTech.', '<p>Bún bò huế là món ăn nổi tiếng miền Trung. Với bún tươi BunTech, bạn sẽ có món bún bò huế chuẩn vị.</p><h2>Nguyên liệu</h2><p>- 500g bún tươi lớn BunTech<br>- 1kg bò bắp<br>- Xương ống heo</p>', 'Công thức nấu ăn', 'https://images.pexels.com/photos/36769199/pexels-photo-36769199.jpeg?auto=compress&cs=tinysrgb&w=800', 15],
  ['BunTech mở rộng giao hàng toàn thành phố', 'Dịch vụ giao hàng mở rộng đến tất cả các quận tại TP. HCM.', '<p>Từ tháng này, BunTech mở rộng khu vực giao hàng đến tất cả các quận tại TP. HCM.</p>', 'Tin tức', 'https://images.pexels.com/photos/6711678/pexels-photo-6711678.jpeg?auto=compress&cs=tinysrgb&w=800', 12],
  ['5 món bún ngon dễ nấu tại nhà', 'Tổng hợp 5 món bún ngon, dễ nấu, phù hợp cho gia đình.', '<p>Bún riêu, bún chả, bún bò huế, bún thịt nướng, bún ốc.</p>', 'Công thức nấu ăn', 'https://images.pexels.com/photos/4541405/pexels-photo-4541405.jpeg?auto=compress&cs=tinysrgb&w=800', 10],
  ['Lợi ích của bún gạo lứt đối với sức khỏe', 'Bún gạo lứt giàu chất xơ, tốt cho người ăn kiêng.', '<p>Bún gạo lứt là lựa chọn lành mạnh, giàu chất xơ, chỉ số đường huyết thấp.</p>', 'Sức khỏe', 'https://images.pexels.com/photos/4541393/pexels-photo-4541393.jpeg?auto=compress&cs=tinysrgb&w=800', 8],
  ['Khuyến mãi tháng 8: Giảm 10% cho đơn sỉ', 'Cơ hội vàng cho đại lý với ưu đãi giảm 10% cho đơn hàng trên 5kg.', '<p>Tháng 8, BunTech giảm 10% cho tất cả đơn hàng sỉ trên 5kg.</p>', 'Khuyến mãi', 'https://images.pexels.com/photos/4541341/pexels-photo-4541341.jpeg?auto=compress&cs=tinysrgb&w=800', 7],
  ['Cách bảo quản bún tươi được lâu', 'Mẹo bảo quản bún tươi trong tủ lạnh được 3-5 ngày.', '<p>Bún tươi nên bảo quản trong tủ lạnh ở 2-4°C.</p>', 'Kiến thức', 'https://images.pexels.com/photos/30705635/pexels-photo-30705635.jpeg?auto=compress&cs=tinysrgb&w=800', 6],
  ['Công thức bún chả Hà Nội chuẩn vị', 'Hướng dẫn làm bún chả Hà Nội với bún tươi BunTech.', '<p>Bún chả Hà Nội là món ăn biểu tượng của thủ đô.</p>', 'Công thức nấu ăn', 'https://images.pexels.com/photos/38802734/pexels-photo-38802734.jpeg?auto=compress&cs=tinysrgb&w=800', 5],
  ['BunTech đạt chứng nhận VSATTP 2024', 'Xưởng bún BunTech đạt chứng nhận vệ sinh an toàn thực phẩm.', '<p>BunTech tự hào đạt chứng nhận VSATTP 2024.</p>', 'Tin tức', 'https://images.pexels.com/photos/6711698/pexels-photo-6711698.jpeg?auto=compress&cs=tinysrgb&w=800', 4],
  ['Miến dong BunTech: Từ nông trại đến bàn ăn', 'Hành trình sản xuất miến dong từ tinh bột khoai lang.', '<p>Miến dong BunTech được làm từ tinh bột khoai lang tự nhiên.</p>', 'Kiến thức', 'https://images.pexels.com/photos/4541405/pexels-photo-4541405.jpeg?auto=compress&cs=tinysrgb&w=800', 3],
  ['Công thức bún riêu cua đồng', 'Bún riêu cua đồng thanh mát, perfect cho mùa hè.', '<p>Bún riêu cua đồng là món ăn thanh mát, dễ nấu.</p>', 'Công thức nấu ăn', 'https://images.pexels.com/photos/4541393/pexels-photo-4541393.jpeg?auto=compress&cs=tinysrgb&w=800', 2],
  ['Top 10 quán bún ngon nhất TP. HCM', 'Gợi ý 10 quán bún ngon nhất Sài Gòn.', '<p>Top 10 quán bún ngon nhất TP. HCM.</p>', 'Tin tức', 'https://images.pexels.com/photos/6711687/pexels-photo-6711687.jpeg?auto=compress&cs=tinysrgb&w=800', 2],
  ['Bún nghệ: Vàng trong ẩm thực Việt', 'Bún nghệ giàu curcumin tốt cho sức khỏe.', '<p>Bún nghệ có màu vàng tự nhiên từ nghệ.</p>', 'Sức khỏe', 'https://images.pexels.com/photos/36769199/pexels-photo-36769199.jpeg?auto=compress&cs=tinysrgb&w=800', 1],
  ['Khuyến mãi: Miễn phí giao hàng cho đơn đầu tiên', 'Khách hàng mới được miễn phí giao hàng.', '<p>Đặt hàng lần đầu tại BunTech, được miễn phí giao hàng!</p>', 'Khuyến mãi', 'https://images.pexels.com/photos/4541341/pexels-photo-4541341.jpeg?auto=compress&cs=tinysrgb&w=800', 1],
  ['Cách chọn gạo làm bún ngon', 'Gạo là nguyên liệu quan trọng nhất quyết định chất lượng bún.', '<p>Để làm bún ngon, cần chọn gạo tẻ có hàm lượng tinh bột cao.</p>', 'Kiến thức', 'https://images.pexels.com/photos/6711678/pexels-photo-6711678.jpeg?auto=compress&cs=tinysrgb&w=800', 0],
  ['Công thức hủ tiếu Nam Vang chuẩn', 'Hủ tiếu Nam Vang với hủ tiếu tươi BunTech.', '<p>Hủ tiếu Nam Vang là món ăn đậm chất Nam Bộ.</p>', 'Công thức nấu ăn', 'https://images.pexels.com/photos/30705635/pexels-photo-30705635.jpeg?auto=compress&cs=tinysrgb&w=800', 0],
  ['Bún tươi vs bún đóng hộp: Nên chọn loại nào?', 'So sánh bún tươi và bún đóng hộp.', '<p>Bún tươi ngon hơn nhưng bảo quản ngắn.</p>', 'Kiến thức', 'https://images.pexels.com/photos/38802734/pexels-photo-38802734.jpeg?auto=compress&cs=tinysrgb&w=800', 0],
  ['Sức khỏe tiêu hóa và bún gạo lứt', 'Bún gạo lứt giàu chất xơ, tốt cho hệ tiêu hóa.', '<p>Bún gạo lứt chứa nhiều chất xơ, giúp cải thiện hệ tiêu hóa.</p>', 'Sức khỏe', 'https://images.pexels.com/photos/4541405/pexels-photo-4541405.jpeg?auto=compress&cs=tinysrgb&w=800', 0],
  ['BunTech ra mắt app đặt hàng online', 'Đặt bún online nhanh chóng, tiện lợi.', '<p>BunTech chính thức ra mắt ứng dụng đặt hàng online.</p>', 'Tin tức', 'https://images.pexels.com/photos/6711698/pexels-photo-6711698.jpeg?auto=compress&cs=tinysrgb&w=800', 0],
  ['Công thức bún ốc đậm vị Bắc', 'Bún ốc Hà Nội với bún tươi BunTech.', '<p>Bún ốc là món ăn đặc sản miền Bắc.</p>', 'Công thức nấu ăn', 'https://images.pexels.com/photos/4541393/pexels-photo-4541393.jpeg?auto=compress&cs=tinysrgb&w=800', 0],
  ['Ung thư dạ dày và thực phẩm an toàn', 'Chọn thực phẩm sạch, không chất bảo quản.', '<p>Thực phẩm sạch là chìa khóa để phòng tránh bệnh tiêu hóa.</p>', 'Sức khỏe', 'https://images.pexels.com/photos/6711687/pexels-photo-6711687.jpeg?auto=compress&cs=tinysrgb&w=800', 0],
  ['Ưu đãi cuối năm: Mua 10 tặng 1', 'Chương trình khuyến mãi cuối năm cho đại lý.', '<p>Mua 10kg bún tươi bất kỳ, tặng 1kg.</p>', 'Khuyến mãi', 'https://images.pexels.com/photos/4541341/pexels-photo-4541341.jpeg?auto=compress&cs=tinysrgb&w=800', 0],
  ['Cách nhận biết bún có chứa hàn the', 'Mẹo nhận biết bún an toàn.', '<p>Bún có hàn the thường quá trắng, giòn bất thường.</p>', 'Kiến thức', 'https://images.pexels.com/photos/36769199/pexels-photo-36769199.jpeg?auto=compress&cs=tinysrgb&w=800', 0],
]
...
const baseBlogPosts: BlogPost[] = blogData.map((b, i) => {
  ...
  return {
    ...
    category: cat,
    title: b[0],
    slug: b[0].toLowerCase().replace(/[^a-z0-9\u00C0-\u1EF9]+/g, '-').replace(/^-|-$/g, ''),
    excerpt: b[1],
    content: b[2],
    image_url: b[4],
    featured_image: b[4],
...
export const mockSystemConfigs: SystemConfig[] = [
  ...
  { id: uid('cfg', 2), key: 'workshop_address', value: '123 Lê Lợi, Phường Bến Nghé, Quận 1, TP. HCM', updated_at: iso(10) },
  ...
  { id: uid('cfg', 6), key: 'workshop_hours', value: '6:00 - 18:00 (T2 - CN)', updated_at: iso(10) },
...
export const mockTopBuyers: TopBuyer[] = [
  { user_id: uid('usr', 53), full_name: customerProfiles[0]?.full_name || 'Phạm Thị Mai', avatar_url: avatarFor(customerProfiles[0]?.full_name || 'Phạm Thị Mai'), total: 18500000 },
  { user_id: uid('usr', 60), full_name: customerProfiles[7]?.full_name || 'Trần Văn Hùng', avatar_url: avatarFor(customerProfiles[7]?.full_name || 'Trần Văn Hùng'), total: 14200000 },
  { user_id: uid('usr', 54), full_name: customerProfiles[1]?.full_name || 'Hoàng Thị Hoa', avatar_url: avatarFor(customerProfiles[1]?.full_name || 'Hoàng Thị Hoa'), total: 9800000 },
  { user_id: uid('usr', 56), full_name: customerProfiles[3]?.full_name || 'Vũ Thị Sỉ', avatar_url: avatarFor(customerProfiles[3]?.full_name || 'Vũ Thị Sỉ'), total: 7600000 },
  { user_id: uid('usr', 57), full_name: customerProfiles[4]?.full_name || 'Bùi Văn Khách', avatar_url: avatarFor(customerProfiles[4]?.full_name || 'Bùi Văn Khách'), total: 5400000 },
]

// ─── Vehicles (80+) ──────────────────────────────────────────────
const vehicleModels = [
  'Honda Wave Alpha', 'Honda Blade', 'Yamaha Sirius', 'Suzuki GD110',
  'Honda Dream', 'Yamaha Exciter', 'Honda PCX', 'Yamaha NVX',
  'Honda Air Blade', 'Yamaha Latte', 'Honda Vision', 'Suzuki Viva',
]
const platePrefixes = ['51A', '59V', '50F', '51K', '59B', '50S', '51L', '59C', '50L', '51M']

export const mockVehicles: Vehicle[] = Array.from({ length: 80 }, (_, i) => {
  const driver = i < 45 ? driverProfiles[i]?.id || null : (i < 50 ? driverProfiles[i]?.id || null : null)
  const status = i < 56 ? VehicleStatus.ACTIVE : (i < 68 ? VehicleStatus.MAINTENANCE : VehicleStatus.INACTIVE)
  return {
    id: uid('veh', i + 1),
    driver_id: driver,
...
export const mockNotifications: Notification[] = Array.from({ length: 120 }, (_, i) => {
  ...
  return {
    id: uid('ntf', i + 1),
    user_id: i < 60 ? driverProfiles[i % driverProfiles.length]?.id || null : customerProfiles[i % customerProfiles.length]?.id || null,
    type,
    title,
...
export const mockMessages: Message[] = Array.from({ length: 120 }, (_, i) => {
  ...
  return {
    id: uid('msg', i + 1),
    sender_id: senderIsDriver ? driverProfiles[i % driverProfiles.length]?.id || null : customerProfiles[i % customerProfiles.length]?.id || null,
    receiver_id: senderIsDriver ? customerProfiles[i % customerProfiles.length]?.id || null : driverProfiles[i % driverProfiles.length]?.id || null,
...
export const mockDeliveryRoutes: DeliveryRoute[] = Array.from({ length: 50 }, (_, i) => {
  ...
  const status: 'PENDING' | 'IN_PROGRESS' | 'COMPLETED' = i < 20 ? 'COMPLETED' : (i < 35 ? 'IN_PROGRESS' : 'PENDING')
  return {
    id: uid('rte', i + 1),
    driver_id: driver?.id || '',
    date: iso(i),
