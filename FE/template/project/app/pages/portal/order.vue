...
import {
  Plus, Trash2, ArrowLeft, Minus, Package, Search, X,
  ShoppingCart, CheckCircle2, MapPin, User, Phone, Truck,
  Tag, Percent,
...
useHead({ title: 'Đặt hàng sỉ - BunTech' })
...
function submitOrder() {
  ...
  setTimeout(() => {
    ...
    success.value = true
    toast.success('Đặt hàng sỉ thành công!')
  }, 1200)
}
...
<template>
  <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12 pb-24 sm:pb-12">
    <!-- Back -->
    <NuxtLink to="/portal" class="inline-flex items-center gap-1.5 text-sm text-gray-500 dark:text-zinc-400 hover:text-surface-foreground transition-colors mb-4 min-h-[44px] px-2 -ml-2 rounded-md">
    ...
    <div class="flex items-center gap-3 mb-2">
      ...
      <div>
        ...
        <p class="text-sm text-gray-500 dark:text-zinc-400">Giá sỉ giảm 10% — đặt số lượng lớn, giao tận nơi</p>
      </div>
    </div>
    ...
    <div class="flex items-center gap-2 px-4 py-3 rounded-xl bg-gradient-to-r from-success-50 to-success-100 dark:from-success-900/20 dark:to-success-800/10 text-success-700 dark:text-success-400 text-sm font-medium mb-6 animate-fade-in-up">
      <Percent class="w-4 h-4" aria-hidden="true" />
    ...
    <template v-if="success">
      <div class="card p-8 sm:p-12 text-center max-w-lg mx-auto animate-fade-in-up">
        ...
        <p class="text-sm text-gray-500 dark:text-zinc-400 mb-2">Mã đơn hàng:</p>
        ...
        <div class="card p-4 mb-6 text-left bg-surface-muted">
          ...
          <div class="flex justify-between text-sm mb-2">
            <span class="text-gray-500 dark:text-zinc-400">Số loại sản phẩm</span>
        ...
        <p class="text-sm text-gray-500 dark:text-zinc-400 mb-6">Chúng tôi sẽ liên hệ để xác nhận đơn hàng và thời gian giao hàng.</p>
        <div class="flex flex-col sm:flex-row gap-3 justify-center">
          <NuxtLink to="/portal"><AppButton variant="outline">Về cổng khách hàng</AppButton></NuxtLink>
              ...
              <div class="grid grid-cols-2 sm:grid-cols-3 gap-3 max-h-[500px] overflow-y-auto pr-1">
                ...
                >
                  ...
                  <div class="flex items-baseline gap-1">
                    ...
                    <span class="text-xs text-gray-400 dark:text-zinc-500 line-through">{{ formatVND(Number(product.price)) }}</span>
                  </div>
        ...
        <div class="space-y-4">
          <div class="card p-4 sticky top-4">
            ...
            <div v-else class="flex flex-col items-center justify-center py-8 text-center">
              ...
              <p class="text-sm text-gray-500 dark:text-zinc-400">Chưa có sản phẩm</p>
            </div>
            ...
            <form v-if="orderItems.length" class="space-y-3 mt-4 pt-4 border-t border-surface-border" @submit.prevent="submitOrder">
              ...
              <AppInput v-model="form.delivery_date" label="Ngày giao hàng (tùy chọn)" placeholder="dd/mm/yyyy" />
              <AppInput v-model="note" label="Ghi chú (tùy chọn)" placeholder="Giao trước 9h sáng" />
