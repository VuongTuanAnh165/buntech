...
import {
  SlidersHorizontal, Grid2x2, LayoutGrid, Package, X,
  Search, ChevronDown,
    ...
    <template v-if="loading">
      <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-6">
        <div v-for="i in 8" :key="i" class="card p-4">
          <AppSkeleton height="h-40" class="mb-3" />
          <AppSkeleton height="h-5" width="w-3/4" class="mb-2" />
          <AppSkeleton height="h-4" width="w-1/2" />
        </div>
      </div>
    </template>
    ...
    <template v-else-if="paginatedProducts.length && viewMode === 'grid'">
      <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-6">
        ...
        >
          ...
          <div class="mt-3">
            <AppButton size="sm" variant="outline" class="w-full group/btn">
              Đặt hàng
