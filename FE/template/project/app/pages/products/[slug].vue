...
import {
  Star, ArrowRight, Package, Truck, ShieldCheck, RotateCcw,
  Share2, Heart, ChevronRight, MessageSquare, Minus, Plus,
...
const product = computed(() =>
  mockProducts.find((p: any) => p.slug === slug && p.status === 'ACTIVE')
)

const reviews = computed(() =>
  mockReviews.filter((r: any) => r.product_id === product.value?.id && r.is_approved)
)

const relatedProducts = computed(() => {
  if (!product.value) return []
  return mockProducts
    .filter((p: any) => p.category_id === product.value!.category_id && p.id !== product.value!.id && p.status === 'ACTIVE')
    .slice(0, 4)
})
...
const avgRating = computed(() => {
  ...
  const sum = reviews.value.reduce((acc: number, r: any) => acc + Number(r.rating), 0)
  return sum / reviews.value.length
})

const ratingDistribution = computed(() => {
  const dist: Record<number, number> = { 5: 0, 4: 0, 3: 0, 2: 0, 1: 0 }
  for (const r of reviews.value as any[]) {
  ...
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12 pb-24 sm:pb-12">
    ...
    <template v-else-if="loading">
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12">
        <div class="space-y-3">
          <AppSkeleton height="h-96" class="rounded-2xl" />
          <div class="flex gap-3">
            <AppSkeleton height="h-20" width="w-20" v-for="i in 4" :key="i" class="rounded-lg" />
          </div>
        </div>
        <div class="space-y-4">
          <AppSkeleton height="h-4" width="w-1/4" />
          <AppSkeleton height="h-10" width="w-3/4" />
          <AppSkeleton height="h-6" width="w-1/3" />
          <AppSkeleton height="h-24" />
          <AppSkeleton height="h-12" width="w-48" />
        </div>
      </div>
    </template>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12">
        ...
        <div>
          ...
          <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 mb-6">
            <AppButton size="lg" variant="outline" class="flex-1 group" @click="addToCart">
              <ShoppingCart class="w-5 h-5" aria-hidden="true" />
              Thêm vào giỏ
            </AppButton>
            <AppButton size="lg" class="flex-1 group" @click="quickOrder">
              ...
              <div class="flex-1 max-w-xs space-y-1">
                <div v-for="star in [5, 4, 3, 2, 1]" :key="star" class="flex items-center gap-2">
                  ...
                  <div class="flex-1 h-1.5 bg-surface-hover rounded-full overflow-hidden">
                    <div class="h-full bg-accent-400 rounded-full transition-all duration-500" :style="{ width: `${reviews.length ? (ratingDistribution[star] / reviews.length) * 100 : 0}%` }" />
                  </div>
    ...
    >
      <div class="flex items-center gap-3">
        ...
        <AppButton size="lg" class="!px-6" @click="quickOrder">
