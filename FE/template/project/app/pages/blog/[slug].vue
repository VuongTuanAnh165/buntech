...
import {
  ArrowLeft, Calendar, User, Clock, Share2, ChevronRight,
  ChevronLeft, Package, Tag, Facebook, Twitter, Link2,
...
const post = computed(() =>
  mockBlogPosts.find((p: any) => p.slug === slug && p.status === 'PUBLISHED')
)

const relatedPosts = computed(() => {
  if (!post.value) return []
  return mockBlogPosts
    .filter((p: any) => p.category_id === post.value!.category_id && p.id !== post.value!.id && p.status === 'PUBLISHED')
    .slice(0, 3)
})

const blogCategory = computed(() =>
  mockBlogCategories.find((c: any) => c.id === post.value?.category_id)
)
...
<template>
  <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
    ...
    <AppErrorState v-if="error" message="Không tìm thấy bài viết" @retry="navigateTo('/blog')" />

    <template v-else-if="loading">
      <AppSkeleton height="h-8" class="mb-4" />
      <AppSkeleton height="h-4" width="w-1/3" class="mb-6" />
      <AppSkeleton height="h-64" class="mb-6" />
      <AppSkeleton height="h-4" class="mb-2" />
      <AppSkeleton height="h-4" class="mb-2" />
      <AppSkeleton height="h-4" width="w-3/4" />
    </template>
    ...
    <template v-else-if="post">
      ...
      <!-- Author bio card -->
      <div class="card p-6 mt-12 flex items-start gap-4">
        <AppAvatar :name="post.author_name" size="lg" />
        <div class="flex-1">
          <div class="flex items-center gap-2 mb-1">
            <p class="font-semibold text-surface-foreground">{{ post.author_name }}</p>
            <AppBadge color="primary" size="sm">Tác giả</AppBadge>
          </div>
