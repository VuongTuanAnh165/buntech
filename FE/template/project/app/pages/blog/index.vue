...
import {
  Package, Search, ChevronRight, ChevronLeft, Calendar,
  User, Clock, X,
...
const allPosts = computed(() =>
  mockBlogPosts.filter((p: any) => p.status === 'PUBLISHED')
)
...
const filteredPosts = computed(() => {
  ...
  if (search.value) {
    ...
    result = result.filter((p: any) => p.title.toLowerCase().includes(q) || p.excerpt?.toLowerCase().includes(q))
  }
    ...
    <template v-if="loading">
      ...
      <div class="card overflow-hidden mb-8">
        <div class="grid grid-cols-1 lg:grid-cols-2">
          <AppSkeleton height="h-64" class="lg:h-full" />
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <div v-for="i in 6" :key="i" class="card p-5">
          <AppSkeleton height="h-40" class="mb-4" />
          <AppSkeleton height="h-5" class="mb-2" />
          <AppSkeleton height="h-4" width="w-3/4" />
        </div>
      </div>
    </template>
