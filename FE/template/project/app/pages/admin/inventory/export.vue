...
useHead({ title: `${t('inventory.export')} - BunTech Admin` })
...
async function submit() {
  ...
  if (quantity.value > maxStock.value) {
    ...
    toast.error(t('inventory.exportExceedsStock'))
    return
  }
  saving.value = true
  try {
...
<template>
  <div>
    <AppBreadcrumb :items="[{ label: t('nav.inventory'), to: '/admin/inventory' }, { label: t('inventory.export') }]" />
    ...
    <h1 class="text-2xl font-bold text-gray-900 mb-6">{{ t('inventory.exportTitle') }}</h1>
    ...
    <div class="max-w-lg bg-white rounded-xl border border-gray-100 p-6 space-y-4">
      ...
      />
      <div v-if="selectedItemId" class="p-3 bg-gray-50 rounded-lg text-sm text-gray-600">
        {{ t('inventory.currentStock') }}: <span class="font-medium">{{ maxStock }} {{ selectedItem?.unit }}</span>
      </div>
      <div>
        ...
        <input
          ...
          :min="1"
          :max="maxStock"
          :class="['w-full rounded-lg border px-3.5 py-2.5 text-sm focus:outline-none focus:ring-2', quantityError ? 'border-danger-500 focus:ring-danger-500' : 'border-gray-300 focus:ring-primary-500']"
        >
        <p v-if="quantityError" class="mt-1 text-sm text-danger-600">{{ t('inventory.exportExceedsStock') }}</p>
      </div>
      <AppInput v-model="note" :label="t('common.note')" />
      <AppButton :loading="saving" block @click="submit">{{ t('inventory.export') }}</AppButton>
    </div>
  </div>
</template>
