...
import {
  ArrowLeft, Copy, Printer, RefreshCw, Clock, Package, Truck, CheckCircle2,
  XCircle, MapPin, Phone, User, Wallet, CreditCard, AlertCircle,
  History, ChevronRight, ShoppingBag, StickyNote, Pencil, UserCheck,
} from 'lucide-vue-next'
import { OrderStatus, ORDER_STATUS_COLORS, ORDER_STATUS_FLOW } from '../../../core/enums'
import type { Order, OrderItem, Profile } from '../../../core/types'
...
const statusFlow = [OrderStatus.PENDING, OrderStatus.PROCESSING, OrderStatus.SHIPPING, OrderStatus.DELIVERED]
...
const paymentState = computed(() => {
  ...
  return { label: 'Chưa thanh toán', color: 'danger' as const }
})
...
const availableDrivers = computed(() => mockProfiles.filter(p => p.role === 'DRIVER' && p.status === 'ACTIVE'))
...
const deliveryTimeline = computed(() => {
  if (isCancelled.value) {
    return [{ label: 'Đã hủy', desc: 'Đơn hàng bị hủy', done: true, color: 'danger' }]
  }
  return [
    { label: 'Tiếp nhận', desc: 'Đã ghi nhận đơn', done: currentStatusIndex.value >= 0, color: 'warning' },
    { label: 'Chuẩn bị', desc: 'Đóng gói sản phẩm', done: currentStatusIndex.value >= 1, color: 'secondary' },
    { label: 'Vận chuyển', desc: 'Đang trên đường giao', done: currentStatusIndex.value >= 2, color: 'accent' },
    { label: 'Giao thành công', desc: 'Khách đã nhận', done: currentStatusIndex.value >= 3, color: 'success' },
  ]
})
...
function assignDriver() {
  ...
  if (drv) {
    ...
    toast.success(`Đã gán tài xế: ${drv.full_name}`)
  }
...
useHead({ title: () => `Đơn hàng #${String(orderId).slice(0, 8)} - BunTech Admin` })
onMounted(loadOrder)
</script>
...
<template>
  <div>
        ...
        <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4 relative">
          <div class="flex-1 min-w-0">
            <div class="flex items-center gap-2 mb-2 flex-wrap">
              ...
              <AppBadge :color="ORDER_STATUS_COLORS[order.status as OrderStatus]" size="md" dot>
            ...
            <div class="flex items-center gap-4 text-sm text-slate-500 dark:text-zinc-400 flex-wrap">
              <span class="flex items-center gap-1.5"><Clock class="w-4 h-4" aria-hidden="true" /> {{ formatDateTime(order.created_at) }}</span>
              <span class="flex items-center gap-1.5"><ShoppingBag class="w-4 h-4" aria-hidden="true" /> {{ items.length }} sản phẩm</span>
            </div>
          </div>
          ...
          <div class="flex flex-col items-start lg:items-end gap-3">
            ...
            <div class="flex flex-wrap gap-2">
              <AppButton size="sm" variant="outline" @click="copyOrder">
                <Copy class="w-4 h-4" /> Sao chép
              </AppButton>
              <AppButton size="sm" variant="outline" @click="printOrder">
                <Printer class="w-4 h-4" /> In
              </AppButton>
              <div class="relative">
                <AppButton size="sm" :loading="changingStatus" @click="showStatusMenu = !showStatusMenu">
                  <RefreshCw class="w-4 h-4" /> Đổi trạng thái
                  <ChevronRight class="w-3.5 h-3.5 transition-transform" :class="showStatusMenu ? 'rotate-90' : ''" />
                </AppButton>
                <Transition name="fade">
                  <div v-if="showStatusMenu" class="absolute right-0 top-full mt-2 z-20 w-48 card p-1.5 shadow-lg">
                    <button
                      ...
                      @click="changeStatus(s as OrderStatus)"
                    >
                      <component :is="statusIcons[s]" class="w-4 h-4" aria-hidden="true" />
                      {{ statusLabels[s as string] }}
                    </button>
                  </div>
                </Transition>
              </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div class="card p-5 animate-fade-in-up" style="animation-delay: 40ms">
            <div class="flex items-center gap-2 mb-4">
              <div class="w-7 h-7 rounded-lg bg-primary-50 dark:bg-primary-900/20 flex items-center justify-center">
                <ShoppingBag class="w-4 h-4 text-primary-600 dark:text-primary-400" aria-hidden="true" />
              </div>
              ...
              >
                <div class="col-span-12 sm:col-span-6 flex items-center gap-3">
                  <div class="w-11 h-11 rounded-lg bg-surface-hover overflow-hidden flex-shrink-0 flex items-center justify-center ring-1 ring-surface-border">
                    ...
                    >
                    <Package v-else class="w-5 h-5 text-gray-300 dark:text-zinc-600" aria-hidden="true" />
                  </div>
                  <div class="min-w-0">
                ...
                <div class="hidden sm:block col-span-2 text-right">
                  <AppBadge color="gray" variant="soft">× {{ item.quantity }}</AppBadge>
                </div>
            ...
            <AppEmptyState v-else title="Không có sản phẩm" description="Đơn hàng này chưa có sản phẩm nào" />
          </div>
          ...
          <div class="card p-5 animate-fade-in-up" style="animation-delay: 80ms">
            <div class="flex items-center gap-2 mb-3">
              <div class="w-7 h-7 rounded-lg bg-warning-50 dark:bg-warning-900/20 flex items-center justify-center">
                <StickyNote class="w-4 h-4 text-warning-600 dark:text-warning-400" aria-hidden="true" />
              </div>
          ...
          <div class="card p-5 animate-fade-in-up" style="animation-delay: 120ms">
            <div class="flex items-center gap-2 mb-5">
              <div class="w-7 h-7 rounded-lg bg-info-50 dark:bg-info-900/20 flex items-center justify-center">
                <Truck class="w-4 h-4 text-info-600 dark:text-info-400" aria-hidden="true" />
              </div>
            ...
            <div v-if="!isCancelled" class="flex items-center justify-between relative mb-2">
              <div class="absolute top-5 left-5 right-5 h-0.5 bg-surface-border" aria-hidden="true" />
              <div
              ...
              >
                <div :class="[
                  ...
                  <component :is="statusIcons[statusFlow[i]]" class="w-5 h-5" aria-hidden="true" />
                </div>
            ...
            <div v-else class="flex items-center gap-3 p-4 rounded-lg bg-danger-50 dark:bg-danger-900/20">
              <div class="w-10 h-10 rounded-full bg-danger-500 text-white flex items-center justify-center flex-shrink-0">
                <XCircle class="w-5 h-5" aria-hidden="true" />
              </div>
              <div>
                <p class="text-sm font-semibold text-danger-700 dark:text-danger-300">Đơn hàng đã bị hủy</p>
                <p class="text-xs text-danger-600 dark:text-danger-400">Quy trình giao hàng đã dừng lại</p>
              </div>
            </div>
          </div>
          ...
          <div class="card p-5 animate-fade-in-up" style="animation-delay: 160ms">
            <div class="flex items-center gap-2 mb-4">
              <div class="w-7 h-7 rounded-lg bg-secondary-50 dark:bg-secondary-900/20 flex items-center justify-center">
                <History class="w-4 h-4 text-secondary-600 dark:text-secondary-400" aria-hidden="true" />
              </div>
            ...
            <ol class="relative space-y-0">
              <li v-for="(h, i) in orderHistory" :key="i" class="flex gap-4 pb-4 last:pb-0 relative">
                <div v-if="i < orderHistory.length - 1" class="absolute left-[15px] top-8 bottom-0 w-px bg-surface-border" aria-hidden="true" />
                <div :class="[
                  'w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 ring-4 ring-surface',
                  h.status === OrderStatus.CANCELLED ? 'bg-danger-50 dark:bg-danger-900/20' :
                  h.status === OrderStatus.DELIVERED ? 'bg-success-50 dark:bg-success-900/20' : 'bg-primary-50 dark:bg-primary-900/20',
                ]">
                  <component :is="statusIcons[h.status]" :class="[
                    'w-4 h-4',
                    h.status === OrderStatus.CANCELLED ? 'text-danger-600 dark:text-danger-400' :
                ...
                <div class="flex-1 min-w-0 pt-0.5">
                  <div class="flex items-center justify-between gap-2">
                    <p class="text-sm font-medium text-surface-foreground">{{ statusLabels[h.status] }}</p>
        ...
        <div class="space-y-6">
          ...
          <div class="card p-5 animate-fade-in-up" style="animation-delay: 40ms">
            ...
            <dl class="space-y-3 text-sm">
              <div class="flex items-center gap-2.5">
                <Phone class="w-4 h-4 text-slate-400 flex-shrink-0" aria-hidden="true" />
                <span class="text-slate-600 dark:text-zinc-300 tabular-nums">{{ customerPhone }}</span>
              </div>
              <div class="flex items-start gap-2.5">
                <MapPin class="w-4 h-4 text-slate-400 flex-shrink-0 mt-0.5" aria-hidden="true" />
          ...
          <div class="card p-5 animate-fade-in-up" style="animation-delay: 80ms">
            <div class="flex items-center gap-2 mb-4">
              <div class="w-7 h-7 rounded-lg bg-primary-50 dark:bg-primary-900/20 flex items-center justify-center">
                <Wallet class="w-4 h-4 text-primary-600 dark:text-primary-400" aria-hidden="true" />
              </div>
            ...
            <div class="flex items-center justify-between mb-4">
              ...
              <AppBadge :color="paymentState.color" dot>{{ paymentState.label }}</AppBadge>
            </div>
            <dl class="space-y-2.5 text-sm">
              ...
              <div class="flex items-center justify-between pt-2.5 border-t border-surface-border">
                <dt class="text-slate-500 dark:text-zinc-400 flex items-center gap-1.5">
                  <CreditCard class="w-3.5 h-3.5" aria-hidden="true" /> Còn nợ
                </dt>
                <dd :class="['font-semibold tabular-nums', remaining > 0 ? 'text-danger-600 dark:text-danger-400' : 'text-surface-foreground']">
                  {{ formatVND(remaining) }}
                </dd>
              </div>
            </dl>
          ...
          <div class="card p-5 animate-fade-in-up" style="animation-delay: 120ms">
            <div class="flex items-center justify-between mb-4">
              <div class="flex items-center gap-2">
                <div class="w-7 h-7 rounded-lg bg-accent-50 dark:bg-accent-900/20 flex items-center justify-center">
                  <Truck class="w-4 h-4 text-accent-600 dark:text-accent-400" aria-hidden="true" />
                </div>
            ...
            <div v-else class="flex items-center gap-2 mb-4 pb-4 border-b border-surface-border text-sm text-slate-400 dark:text-zinc-500">
              <AlertCircle class="w-4 h-4" aria-hidden="true" /> Chưa gán tài xế
            </div>
            ...
            <div class="space-y-3">
              ...
              >
                <div :class="['w-2.5 h-2.5 rounded-full flex-shrink-0', step.done ? 'bg-success-500' : 'bg-surface-border']" aria-hidden="true" />
                <div class="flex-1 min-w-0">
                ...
                <CheckCircle2 v-if="step.done" class="w-4 h-4 text-success-500 flex-shrink-0" aria-hidden="true" />
              </div>
            </div>
          </div>
        </div>
    ...
    <AppEmptyState v-else title="Không tìm thấy đơn hàng" description="Đơn hàng không tồn tại hoặc đã bị xoá" />

    <!-- Assign Driver Modal -->
    <AppModal v-model="showAssignDriver" title="Gán tài xế" size="sm">
      <div class="space-y-2 max-h-80 overflow-y-auto">
        ...
        >
          <AppAvatar :name="drv.full_name" size="sm" />
          <div class="min-w-0">
          ...
          <CheckCircle2 v-if="selectedDriverId === drv.id" class="w-5 h-5 text-primary-600 dark:text-primary-400 flex-shrink-0 ml-auto" aria-hidden="true" />
        </button>
      </div>
      <template #footer>
        <AppButton variant="ghost" @click="showAssignDriver = false">Huỷ</AppButton>
        <AppButton :disabled="!selectedDriverId" @click="assignDriver">Gán tài xế</AppButton>
      </template>
    </AppModal>
  </div>
</template>
