...
const stockState = computed(() => {
  if (stock.value <= 0)
    return { label: 'Hết hàng', color: 'error', tailwind: 'bg-error-500', pct: 0 }
  if (stock.value <= 50)
    return { label: 'Sắp hết', color: 'warning', tailwind: 'bg-warning-500', pct: stockPct.value }
  if (stock.value <= 200)
    return { label: 'Khá', color: 'info', tailwind: 'bg-info-500', pct: stockPct.value }
...
const analyticsCards = computed(() => [
  {
    label: 'Đã bán',
    value: formatNumber(analytics.totalSold) + ' ' + unit.value,
  ...
  {
    label: 'Doanh thu',
    value: formatVND(analytics.revenue),
  ...
  },
  {
    ...
    value: formatVND(analytics.avgOrderValue),
...
const specs = computed(() => [
  { icon: 'i-lucide-tag', label: 'Danh mục', value: categoryName.value },
  {
    ...
    label: 'Giá bán',
    value: `${formatVND(price.value)} / ${unit.value}`
  },
  {
    icon: 'i-lucide-layers',
    label: 'Tồn kho',
    value: `${formatNumber(stock.value)} ${unit.value}`
  },
  {
  ...
  },
  {
    ...
    label: 'Ngày tạo',
    value: formatDate(product.value?.created_at || '') || '—'
  },
  {
    icon: 'i-lucide-calendar',
    label: 'Cập nhật',
    value: formatDate(product.value?.updated_at || '') || '—'
  }
])
...
function saveReorder() {
  toast.add({
    title: `Đã cập nhật điểm đặt hàng lại: ${reorderPoint.value} ${unit.value}`,
    color: 'success'
  })
}
...
<template>
  <div>
    ...
    <template v-else-if="loading">
      <div class="card animate-fade-in-up mb-6 p-6">
        <div class="flex flex-col gap-6 sm:flex-row">
          <div class="skeleton h-56 w-full flex-shrink-0 rounded-2xl sm:w-56" />
      ...
      <div class="skeleton mb-6 h-10 w-72 rounded-lg" />
    ...
    <template v-else-if="product">
      ...
      <div class="card animate-fade-in-up relative mb-6 overflow-hidden p-6">
        <div
          class="bg-primary-500/5 dark:bg-primary-400/5 absolute -top-12 -right-12 h-48 w-48 rounded-full blur-2xl"
          aria-hidden="true"
        />
        <div class="relative flex flex-col gap-6 lg:flex-row">
          <!-- Image -->
          <div class="w-full flex-shrink-0 lg:w-60 xl:w-72">
            <div
              class="bg-surface-hover ring-surface-border flex aspect-square items-center justify-center overflow-hidden rounded-2xl ring-1"
            >
              <NuxtImg
              ...
              <span
                v-else
                class="i-lucide-image h-16 w-16 text-gray-300 dark:text-zinc-600"
                aria-hidden="true"
              />
            </div>
          </div>
          ...
          <div class="flex min-w-0 flex-1 flex-col">
            ...
            <p class="mb-4 font-mono text-sm text-slate-500 dark:text-zinc-400">
              #{{ String(productId).slice(0, 8) }}
            </p>

            <div class="mb-4 flex items-baseline gap-2">
              <span
                class="text-primary-600 dark:text-primary-400 text-3xl font-bold tabular-nums"
                >{{ formatVND(price) }}</span
              >
            ...
            <div class="mb-4">
              <div class="mb-1.5 flex items-center justify-between text-sm">
                <span class="flex items-center gap-1.5 text-slate-500 dark:text-zinc-400">
                  <span class="i-lucide-boxes h-4 w-4" aria-hidden="true" /> Tồn kho
                </span>
                <span class="text-surface-foreground font-semibold tabular-nums"
                  >{{ formatNumber(stock) }} {{ unit }}</span
                >
              </div>
              <div class="bg-surface-hover h-2 overflow-hidden rounded-full">
                <div
                  :class="['h-full rounded-full transition-all duration-500', stockState.tailwind]"
                  :style="{ width: `${stockState.pct}%` }"
                />
              </div>
            </div>

            <p class="line-clamp-2 flex-1 text-sm text-slate-600 dark:text-zinc-300">
              {{ description }}
            </p>
            ...
            <div class="mt-5 flex flex-wrap gap-2">
              ...
              <UButton size="sm" color="error" icon="i-lucide-trash-2" @click="confirmDelete"
                >Xoá</UButton
              >
              <UButton size="sm" color="neutral" variant="outline" icon="i-lucide-external-link"
                >Xem trang bán</UButton
              >
            </div>
          </div>
        </div>
      ...
      <Transition name="fade" mode="out-in">
        <!-- ===== Overview Tab ===== -->
        <div v-if="activeTab === 'overview'" key="overview" class="animate-fade-in-up space-y-6">
          <div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
            <div class="card stagger-item p-5 lg:col-span-2">
              <div class="mb-3 flex items-center gap-2">
                ...
                >
                  <span
                    class="i-lucide-package text-primary-600 dark:text-primary-400 h-4 w-4"
                    aria-hidden="true"
                  />
                </div>
                <h2 class="text-surface-foreground text-sm font-semibold">Mô tả sản phẩm</h2>
              </div>
            ...
            <div class="card stagger-item p-5" style="animation-delay: 40ms">
              <div class="mb-3 flex items-center gap-2">
                <div
                  class="bg-secondary-50 dark:bg-secondary-900/20 flex h-7 w-7 items-center justify-center rounded-lg"
                >
                  <span
                    class="i-lucide-tag text-secondary-600 dark:bg-secondary-400 h-4 w-4"
                    aria-hidden="true"
                  />
                </div>
                <h2 class="text-surface-foreground text-sm font-semibold">Thông số</h2>
              </div>
              <dl class="space-y-3">
                <div
                  v-for="spec in specs"
                ...
                >
                  <dt class="flex items-center gap-1.5 text-xs text-slate-500 dark:text-zinc-400">
                    <span :class="['h-3.5 w-3.5', spec.icon]" aria-hidden="true" />
                    {{ spec.label }}
                  </dt>
          ...
          <!-- Image gallery -->
          <div class="card stagger-item p-5" style="animation-delay: 80ms">
            <h2 class="text-surface-foreground mb-4 text-sm font-semibold">Hình ảnh sản phẩm</h2>
            <div class="grid grid-cols-2 gap-3 sm:grid-cols-4">
              <div
                class="bg-surface-hover ring-surface-border aspect-square overflow-hidden rounded-lg ring-1"
              >
                <NuxtImg
                  v-if="productImageUrl"
                  :src="productImageUrl"
                ...
                <span
                  v-else
                  class="i-lucide-package m-auto mt-12 block h-8 w-8 text-gray-300 dark:text-zinc-600"
                  aria-hidden="true"
                />
              </div>
              <div
                v-for="i in 3"
                :key="i"
                class="bg-surface-hover ring-surface-border flex aspect-square items-center justify-center overflow-hidden rounded-lg ring-1"
              >
                <span
                  class="i-lucide-package h-8 w-8 text-gray-300 dark:text-zinc-600"
                  aria-hidden="true"
                />
              </div>
            </div>
          ...
          <!-- Related products -->
          <div class="card stagger-item p-5" style="animation-delay: 120ms">
            <div class="mb-4 flex items-center justify-between">
              <h2 class="text-surface-foreground text-sm font-semibold">Sản phẩm liên quan</h2>
              <NuxtLink
                to="/admin/products"
              ...
              >
                Xem tất cả <span class="i-lucide-chevron-right h-3 w-3" aria-hidden="true" />
              </NuxtLink>
            </div>
            <div v-if="relatedProducts.length" class="grid grid-cols-2 gap-4 sm:grid-cols-4">
              <NuxtLink
                v-for="rp in relatedProducts"
                :key="rp.id"
                :to="`/admin/products/${rp.id}`"
                class="card card-hover group p-3"
              >
                <div
                  class="bg-surface-hover mb-2 flex aspect-square items-center justify-center overflow-hidden rounded-lg"
                >
                  <NuxtImg
                    v-if="rp.image_url"
                    :src="rp.image_url"
                ...
                <p class="text-surface-foreground truncate text-sm font-medium">{{ rp.name }}</p>
                <p
                  class="text-primary-600 dark:text-primary-400 mt-0.5 text-xs font-semibold tabular-nums"
                >
                  {{ formatVND(rp.price) }}
                </p>
                <div class="mt-1">
            ...
            <BaseEmptyState
              ...
              description="Không tìm thấy sản phẩm cùng danh mục"
              icon="i-lucide-package"
            />
          </div>
        </div>
        ...
        >
            ...
            <div class="card stagger-item p-5 lg:col-span-2">
              ...
              <div class="mb-2 flex items-end justify-between">
                <div class="flex items-baseline gap-2">
                  <span class="text-surface-foreground text-4xl font-bold tabular-nums">{{
                    formatNumber(stock)
                  }}</span>
                  <span class="text-sm text-slate-500 dark:text-zinc-400">{{ unit }}</span>
                </div>
                <span class="text-xs text-slate-400 dark:text-zinc-500"
                  >Tối đa {{ formatNumber(MAX_STOCK) }} {{ unit }}</span
                >
              </div>
              <div class="bg-surface-hover mb-2 h-3 overflow-hidden rounded-full">
                <div
                  :class="['h-full rounded-full transition-all duration-700', stockState.tailwind]"
                  :style="{ width: `${stockState.pct}%` }"
                />
              </div>
              <div class="border-surface-border mt-5 grid grid-cols-3 gap-3 border-t pt-5">
                <div class="text-center">
                  <p class="mb-1 text-xs text-slate-500 dark:text-zinc-400">Đã bán (30 ngày)</p>
                  <p class="text-surface-foreground text-lg font-bold tabular-nums">840</p>
                </div>
                <div class="border-surface-border border-x text-center">
                  <p class="mb-1 text-xs text-slate-500 dark:text-zinc-400">Nhập (30 ngày)</p>
                  <p class="text-success-600 dark:text-success-400 text-lg font-bold tabular-nums">
                    1,050
                  </p>
                </div>
                <div class="text-center">
                  <p class="mb-1 text-xs text-slate-500 dark:text-zinc-400">Hao hụt</p>
            ...
            <div class="space-y-6">
              <div class="card stagger-item p-5" style="animation-delay: 40ms">
                <div class="mb-3 flex items-center gap-2">
                  ...
                  >
                    <span
                      class="i-lucide-dollar-sign text-success-600 dark:text-success-400 h-4 w-4"
                      aria-hidden="true"
                    />
                  </div>
                  <h2 class="text-surface-foreground text-sm font-semibold">Giá trị tồn kho</h2>
                </div>
                <p class="text-surface-foreground text-2xl font-bold tabular-nums">
                  {{ formatVND(stockValue) }}
                </p>
                <p class="mt-1 text-xs text-slate-500 dark:text-zinc-400">
                  {{ formatNumber(stock) }} {{ unit }} × {{ formatVND(price) }}
                </p>
              </div>
              ...
              <div class="card stagger-item p-5" style="animation-delay: 80ms">
                <div class="mb-3 flex items-center gap-2">
                  ...
                  >
                    <span
                      class="i-lucide-alert-triangle text-warning-600 dark:text-warning-400 h-4 w-4"
                      aria-hidden="true"
                    />
                  </div>
                  <h2 class="text-surface-foreground text-sm font-semibold">Điểm đặt hàng lại</h2>
                </div>
                <p class="mb-3 text-xs text-slate-500 dark:text-zinc-400">
                  Cảnh báo khi tồn kho giảm xuống mức này
                </p>
                <UInput v-model="reorderPoint" type="number">
                  <template #trailing
                    ><span class="text-xs text-slate-500">{{ unit }}</span></template
                  >
                </UInput>
                <UButton
                  size="sm"
          ...
          <div class="card stagger-item p-5" style="animation-delay: 120ms">
            <div class="mb-4 flex items-center gap-2">
              ...
              >
                <span
                  class="i-lucide-activity text-info-600 dark:text-info-400 h-4 w-4"
                  aria-hidden="true"
                />
              </div>
              <h2 class="text-surface-foreground text-sm font-semibold">Lịch sử biến động kho</h2>
            </div>
        ...
        <!-- ===== Reviews Tab ===== -->
        <div v-else-if="activeTab === 'reviews'" key="reviews" class="animate-fade-in-up space-y-6">
          <div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
            <div class="card stagger-item p-5">
              <div class="mb-4 flex flex-col items-center text-center">
                <div class="mb-2 flex items-center gap-1">
                  <span
                    class="i-lucide-star text-accent-400 fill-accent-400 h-7 w-7"
                    aria-hidden="true"
                  />
                  <span class="text-surface-foreground text-4xl font-bold tabular-nums">{{
                    avgRating.toFixed(1)
              ...
              <div class="space-y-2">
                <div v-for="r in ratingDistribution" :key="r.star" class="flex items-center gap-2">
                  <span
                    class="flex w-6 items-center gap-0.5 text-xs text-slate-500 dark:text-zinc-400"
                  >
                    {{ r.star }}
                    <span
                      class="i-lucide-star text-accent-400 fill-accent-400 h-3 w-3"
                      aria-hidden="true"
                    />
                  </span>
                  <div class="bg-surface-hover h-2 flex-1 overflow-hidden rounded-full">
                    <div
                      class="bg-accent-400 h-full rounded-full transition-all duration-500"
                      :style="{ width: `${totalReviews ? (r.count / totalReviews) * 100 : 0}%` }"
                    />
                  </div>
                  <span
                    class="w-6 text-right text-xs text-slate-500 tabular-nums dark:text-zinc-400"
                    >{{ r.count }}</span
                  >
                </div>
              </div>
            ...
            <div class="card stagger-item p-5 lg:col-span-2" style="animation-delay: 40ms">
              <h2 class="text-surface-foreground mb-4 text-sm font-semibold">Đánh giá gần đây</h2>
              <template v-if="reviews.length">
                <div class="space-y-4">
                  <div
                    v-for="review in reviews"
                    :key="review.id"
                    class="border-surface-border flex gap-3 border-b pb-4 last:border-0 last:pb-0"
                  >
                      ...
                      <div class="mb-1.5 flex items-center gap-0.5">
                        <span
                          ...
                          :class="[
                            'h-3.5 w-3.5',
                            i <= review.rating
                              ? 'i-lucide-star text-accent-400 fill-accent-400'
                              : 'i-lucide-star fill-none text-gray-200 dark:text-zinc-600'
                          ]"
                          aria-hidden="true"
                        />
                      </div>
              ...
              <BaseEmptyState
                ...
                description="Khách hàng chưa đánh giá sản phẩm này"
        ...
        >
          <div class="grid grid-cols-2 gap-4 lg:grid-cols-4">
            <div
              v-for="(card, i) in analyticsCards"
              :key="card.label"
              class="card card-hover stagger-item relative overflow-hidden p-4"
            ...
            >
              ...
              <p class="mb-0.5 text-xs font-medium text-slate-500 dark:text-zinc-400">
                {{ card.label }}
              </p>
              <p
                class="text-surface-foreground truncate text-lg font-bold tracking-tight tabular-nums"
              >
                {{ card.value }}
              </p>
            </div>
          ...
          <div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
            <div class="card stagger-item p-5 lg:col-span-2" style="animation-delay: 40ms">
              <div class="mb-5 flex items-center justify-between">
                <div>
                  <h2 class="text-surface-foreground text-sm font-semibold">Doanh số theo tháng</h2>
                  <p class="mt-0.5 text-xs text-slate-500 dark:text-zinc-400">6 tháng gần nhất</p>
                </div>
                <div
                  class="text-success-600 dark:text-success-400 bg-success-50 dark:bg-success-900/20 flex items-center gap-1.5 rounded-md px-2.5 py-1 text-sm font-medium"
                >
                  <span class="i-lucide-trending-up h-3.5 w-3.5" aria-hidden="true" />
                  <span class="tabular-nums">{{ formatVND(analytics.revenue) }}</span>
                </div>
              </div>
              <div class="flex h-56 items-end justify-between gap-3 pt-4">
                <div
                ...
                >
                  <div class="relative flex h-full w-full flex-col justify-end">
                    <div
                      class="text-surface-foreground bg-surface-hover ring-surface-border absolute -top-6 left-1/2 z-10 -translate-x-1/2 rounded px-1.5 py-0.5 text-[10px] font-semibold whitespace-nowrap tabular-nums opacity-0 shadow-sm ring-1 transition-opacity group-hover:opacity-100"
                    >
                      {{ formatVND(m.sales) }}
                    </div>
                    <div
                      class="from-primary-500 to-primary-400 group-hover:from-primary-600 group-hover:to-primary-500 w-full rounded-t-md bg-gradient-to-t transition-all duration-500"
            ...
            <div class="card stagger-item p-5" style="animation-delay: 80ms">
              <div class="mb-3 flex items-center gap-2">
                <span class="i-lucide-users text-accent-500 h-4 w-4" aria-hidden="true" />
                <h2 class="text-surface-foreground text-sm font-semibold">Khách mua nhiều</h2>
              </div>
              <div class="space-y-1">
                <div
                  v-for="(c, i) in topCustomers"
                  :key="c.name"
                  class="hover:bg-surface-hover flex items-center gap-3 rounded-lg p-2 transition-colors"
                >
                  <div class="relative">
                  ...
                  <div class="min-w-0 flex-1">
                    <p class="text-surface-foreground truncate text-sm font-medium">{{ c.name }}</p>
                    <p class="text-xs text-slate-500 tabular-nums dark:text-zinc-400">
                      {{ c.orders }} đơn · {{ formatVND(c.total) }}
                    </p>
                  </div>
                </div>
              </div>
